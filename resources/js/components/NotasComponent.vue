<template>
    <div class="container">
        ...
        <form @submit.prevent="agregarNota" v-else>
            <h3>Agregar nota</h3>
            <input type="text" class="form-control mb-2"
                placeholder="Nombre de la nota" v-model="nota.nombre">
            <input type="text" class="form-control mb-2"
    ...
    export default {
        data() {
            return {
                ...
                path:"/laravel/public/" // Just Testing
            }
        },
        created(){

            axios.get(this.path+"/notas").then(res=>{
                this.notas = res.data;
            })
        },
        methods:{
            ...
            agregarNota(){
                ...
                axios.post(this.path+'/notas', notaNueva)
                    .then((res) =>{
            ...
            editarNota(nota){
                ...
                axios.put(this.path+`/notas/${nota.id}`, params)
                    .then(res=>{
            ...
            eliminarNota(nota, index){
                ...
                if(confirmacion){
                    axios.delete(this.path+`/notas/${nota.id}`)
                    .then(()=>{
